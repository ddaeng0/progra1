...
int main() {
    ...
    //1. Generar un arreglo de 30 elementos que contenga los c�digos de los vendedores. Se sabe que los c�digos
    //de los vendedores empezar�n en el valor 201.
    int filas{30};
    int* codigos = new int[filas];
    for (int i = 0; i < filas; ++i)
        codigos[i] = 201 + i;
    
    //2. Generar una matriz de 30 x 3, que contenga de forma aleatoria la informaci�n de los minutos de llamadas
    //a fijo y celulares as� como el n�mero de mensajes de texto de cada vendedor.
    int columnas{3};
    ...
    //3. Determinar el c�digo o c�digos de los vendedores que tienen la mayor cantidad de minutos de llamadas
    // a celulares.
...
void determinarMontosPorExceso(int* codigos, int** informacion, int filas, int columnas) {
    ...
    Plan: 
    - 20 minutos de llamadas a tel�fono fijo
    - 40 minutos a celulares 
    - 20 mensajes de texto

    Costo por exceso:
    - 0.35 soles por minuto para tel�fono fijo
